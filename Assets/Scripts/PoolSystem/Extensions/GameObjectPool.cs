...
public class GameObjectPool
{
    ...
    {
        // Limpiar referencias destruidas (opcional pero recomendado)
        _poolContent.RemoveAll(obj => obj == null);

        foreach (GameObject objInstance in _poolContent)
        {
            if (objInstance != null && !objInstance.activeInHierarchy)
            {
        ...
        if (isExpandible)
        {
            GameObject obj = GameObject.Instantiate(prefab, _pool.transform);
            _poolContent.Add(obj);
            Debug.Log("Nuevo Objeto: " + obj.name);
            return obj;
        }
